...
def get_default_dataset_module(dataset, data_dir=None, cluster_config=None):
    is_on_cluster = False
    canonical_module = f"nemo_skills.dataset.{dataset}"

    if data_dir is None:
        data_path = "/nemo_run/code/nemo_skills/dataset"
        dataset_module = importlib.import_module(canonical_module)
    ...
    if cluster_config is None or cluster_config["executor"] == "none":
        try:
            ...
            module_file = getattr(dataset_module, "__file__", "")
            module_realpath = os.path.realpath(module_file) if module_file else ""
            data_realpath = os.path.realpath(data_dir)
            try:
                inside_data_dir = module_realpath and os.path.commonpath([module_realpath, data_realpath]) == data_realpath
            except ValueError:
                inside_data_dir = False
            if not module_file or module_file.startswith("<") or not inside_data_dir:
                dataset_module = importlib.import_module(canonical_module)
    else:
        if cluster_config["executor"] == "local":
            unmounted = get_unmounted_path(cluster_config, data_dir)
            try:
                with add_to_path(unmounted):
                    dataset_module = importlib.import_module(dataset)
            except ModuleNotFoundError:
                dataset_module = importlib.import_module(canonical_module)
            else:
                module_file = getattr(dataset_module, "__file__", "")
                module_realpath = os.path.realpath(module_file) if module_file else ""
                data_realpath = os.path.realpath(unmounted)
                try:
                    inside_data_dir = module_realpath and os.path.commonpath([module_realpath, data_realpath]) == data_realpath
                except ValueError:
                    inside_data_dir = False
