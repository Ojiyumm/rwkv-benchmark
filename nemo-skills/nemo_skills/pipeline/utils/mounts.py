...
def check_if_mounted(cluster_config, path_to_check):
    ...
    if cluster_config["executor"] in ("none", "local"):
        # no mounts needed for local executors
        return
...
def check_mounts(
    ...
    # Fast-path for local executor: no mount checks needed, just resolve placeholders.
    if cluster_config["executor"] == "local":
        ...
        if log_dir:
            log_dir = _resolve_path_placeholders(log_dir)
        if new_paths:
            return *new_paths, log_dir
        return log_dir

    # Compute directory of all files for remote executors
    remote_dir_list = []
...
def get_mounted_path(cluster_config: dict, path: str):
    ...
    """
    if cluster_config["executor"] in ("none", "local"):
        # local executors operate directly on host paths
        return path
    if path is None:
        return None
    ...
    if mount_path is None:
        mounts_list = cluster_config.get("mounts", [])
        raise ValueError(
...
def get_unmounted_path(cluster_config: dict, path: str):
    ...
    if mount_path is None:
        mounts_list = cluster_config.get("mounts", []) + ["/nemo_run/code:/nemo_run/code"]
        raise ValueError(
            f"Could not find a mount path for the file path `{path}`. Check cluster config. Below paths are mounted: \n"
            f"{mounts_list}"
        )
